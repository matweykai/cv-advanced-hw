...
class ArcFaceLoss(nn.Module):
    def __init__(self, margin, scale, emb_dim, class_count):
        super().__init__()

        self.weights = nn.Parameter(torch.randn(emb_dim, class_count))
    ...
    def forward(self, embeddings, labels):
        cos_theta = F.linear(embeddings, nn.functional.normalize(self.weights).T, bias=None)
        cos_theta = cos_theta.clip(-1+1e-7, 1-1e-7)

        M = F.one_hot(labels, num_classes=self.class_count) * self.margin
...
def train_one_epoch(model, arc_face_loss, dataloader, optimizer, device, sum_writer: SummaryWriter):
    model.train()
    running_loss = 0.0
    ...
    for batch_idx, batch in enumerate(dataloader):
        ...
        loss = arc_face_loss(model_pred, labels)
...
def validate_recall_at_k(model, dataloader, k, device):
    ...
    distances = 1 - torch.mm(embeddings_all, embeddings_all.T)  # batch x batch
...
def main(args):
    ...
    criterion = ArcFaceLoss(margin, scale, 128, train_samples_class_count).to(device)
