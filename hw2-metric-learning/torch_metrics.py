...
def train_one_epoch(model, dataloader, optimizer, device, criterion, miner: DistanceWeightedMiner, sum_writer: SummaryWriter):
    model.train()
    running_loss = 0.0
...
def validate(model, dataloader, criterion, miner, device):
    model.eval()
    running_loss = 0.0
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            hard_pairs = miner(anchor_out, label)

            loss = criterion(anchor_out, label, hard_pairs)
            running_loss += loss.item()
...
def main(args):
    ...
    for epoch in range(num_epochs):
        ...
        train_loss = train_one_epoch(model, train_loader, optimizer, device, criterion, miner, writer)
        val_loss = validate(model, val_loader, criterion, miner, device)
